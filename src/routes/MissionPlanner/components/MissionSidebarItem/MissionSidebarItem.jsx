...
import styles from './MissionSidebarItem.scss';
...
export class MissionSidebarItem extends Component {
  constructor(props) {
    super(props);
  ...
  handleNumberChange(name, event) {
    ...
    if (value.match(/^-?\d*(\.\d*)?$/)) {
      ...
      if (coord > -1) {
        missionItem.coordinate[coord] = getFloatValue(value);
      } else {
        missionItem[name] = getFloatValue(value);
      }
  ...
  render() {
    ...
    return (
      <form>
        <Grid styleName="mission-sidebar-item">
          <Row>
          ...
          <div styleName={this.state.fullBody}>
            ...
            <div>
              ...
              <Row styleName="row">
                <Col sm={4}>
                  <span styleName="label">{this.props.command === 16 ? 'Hold time:' : 'Param1:'}</span>
                </Col>
                <Col sm={8}>
              ...
              <Row styleName="row">
                <Col sm={4}>
                  <span styleName="label">{this.props.command === 16 ? 'Heading:' : 'Param4:'}</span>
                </Col>
                <Col sm={8}>
...
const commandPropType = PropTypes.oneOf(commands.map(_.property('value')));

const framePropType = PropTypes.oneOf(frames.map(_.property('value')));

MissionSidebarItem.propTypes = {
  ...
  param4: PropTypes.number.isRequired,
  command: commandPropType.isRequired,
  frame: framePropType.isRequired,
  onUpdate: PropTypes.func.isRequired,
  onDelete: PropTypes.func.isRequired,
};

export default CSSModules(MissionSidebarItem, styles);
